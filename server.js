...
app.get("/api/admin/products",async(req,res)=>{let r=await pool.query("select p.*,c.name category from products p join categories c on c.id=p.category_id order by p.id desc");res.json(r.rows)});
